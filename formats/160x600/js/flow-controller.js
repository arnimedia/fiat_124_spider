...
$.extend(FlowController.prototype, {
       ...
       init:function()
    {
		...
		function car_tires(){
			...
			car_tl
				.set('#tire_front',{css:{transform:'skewX(-2.7deg) scale(.22,.36)'}})
				.set('#tire_back',{css:{transform:'skewX(-2.1deg) scale(.13,.28)'}})
        ...
        // MAIN TIMELINE
                ...
                tl.set('#navy_corner', {scale: 2, x: -110, y: 150});
				...
				tl.staggerFrom(subtitleLine1SplitText.words, 0.4, {x:docWidth, ease:Power2.easeOut},0.025, (label + "+=2"));
				...
				tl.staggerTo(subtitleLine1SplitText.words, 0.4, {opacity:0},0, label+"+=4");
